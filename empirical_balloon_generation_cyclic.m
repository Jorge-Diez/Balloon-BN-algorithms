    ...
    for experiment_n = 1:10
        ...
        switch testrun
            ...
            case {2}
                ...
                ntpts=nsecs/res;
                exp_name = "tr750ms";
                load('../NETSIM_filters/filter_tr750ms.mat');
            case {3}
                tr=3.0;
                nscans=nsecs/tr;
                res=1/200;           % 1/20    raw data resolution in seconds
        ...
        experiment_dir_name = 'Cycle-Empirical-Resting-State_' + exp_name + '/run' + string(experiment_n);
        ...
        for n = 1:Nsub/2
            ...
            for I = 1:5:5*Nsubnets
                ...
                for i=I:I+7, a(i,i)=-1; end;    % no self-connections
                
                a(1,3) = random(pd_truncated, 1);
                a(1,5) = random(pd_truncated, 1);
                a(2,3) = random(pd_truncated, 1);
                a(2,5) = random(pd_truncated, 1);
                a(3,4) = random(pd_truncated, 1);
                a(3,6) = random(pd_truncated, 1);
                a(3,8) = random(pd_truncated, 1);
                a(6,7) = random(pd_truncated, 1);
                ...
                c=eye(8*Nsubnets);     %%%%%% external inputs
        ...
        for subject_number = 1:Nsub
            ...
            disp("Subject " + subject_number  + " finished of run " + experiment_n + " of empirical cyclic structure " + exp_name )
