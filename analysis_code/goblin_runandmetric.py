...
NUM_EXPERIMENTS = 24800

#where is all the bold data
DATA_FOLDER = r"D:\My Documents\MUIA\TFM\CasualFMRI\NETSIM_DATA_FORGOBLINTODELET"



METRIC_SAVE_LOCATION_GOBLIN = r"D:\My Documents\MUIA\TFM\CasualFMRI\METRICS\GOBLIN"
...
for (directory) in natsorted(os.listdir(DATA_FOLDER)):
    ...
    for (subdirectory) in natsorted(os.listdir(main_experiment_directory)):
        ...
        for (run) in natsorted(os.listdir(experiment_directory)):
            ...
            colnames= detect_colnames(directory)
            ...
            for (subject) in natsorted(os.listdir(bold_data_directory)):
                ...
                data = pd.read_csv (subject_data, names = colnames, header=None)
                
                m = Gobnilp() 
                m.learn(data, data_type='continuous',score='BGe',palim=None, plot=False, output_stem = "stemerino", output_ext = ('plain',))

                all_lines = obtain_all_connections('stemerino.plain', nr_nodes)
            ...
            direction_metrics_goblin = METRIC_SAVE_LOCATION_GOBLIN + "\\" + directory +  "\\" + subdirectory + "\\" + run

            os.makedirs(direction_metrics_goblin)
            
            with open( direction_metrics_goblin + "\\raw_metrics" + '.pickle', 'wb') as handle:
            ...
            with open( direction_metrics_goblin + "\\mean_metrics" + '.pickle', 'wb') as handle:
                pickle.dump(mean_metrics, handle, protocol=pickle.HIGHEST_PROTOCOL)
                
            run_count = run_count + 1    

            shutil.rmtree(experiment_directory_run, ignore_errors = True)
            
            if (run == 'run10'):
                shutil.rmtree(experiment_directory, ignore_errors = True)
                
                if ((subdirectory == 'Resting-State_tr3000ms') or (subdirectory == 'Boxcar_Resting-State_tr3000ms') or (subdirectory == 'Empirical-Resting-State_tr3000ms')  ):
                    shutil.rmtree(main_experiment_directory, ignore_errors = True)
