...
DATA_SAVE_LOCATION = r"D:\My Documents\MUIA\TFM\CasualFMRI\NETSIM_CONCATENATED_NOCENTERING"
    ...
    for (subdirectory) in natsorted(os.listdir(main_experiment_directory)):
        ...
        for (run) in natsorted(os.listdir(experiment_directory)):
            ...
            for (subject) in natsorted(os.listdir(bold_data_directory)):
                ...
                sub_data = pd.read_csv(subject_data, header=None) 
                
                
                all_data = np.vstack((all_data, sub_data)) if all_data.size else sub_data
                
                
            
            place_to_save = direction_new_bold + "\\concatenated_bold_nozscore.csv"
                
            pd.DataFrame(all_data).to_csv(place_to_save, header=False, index=False)
