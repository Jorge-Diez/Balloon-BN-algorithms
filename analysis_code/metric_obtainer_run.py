...
SIM_DATA_FOLDER = r"D:\My Documents\MUIA\TFM\CasualFMRI\NETSIM_ALGORITHM_RESULTS\iMAGES"

#Where BOLD data will be copied
METRIC_SAVE_LOCATION = r"D:\My Documents\MUIA\TFM\CasualFMRI\METRICS\iMAGES"
...
for (directory) in natsorted(os.listdir(SIM_DATA_FOLDER)):
    ...
    for (subdirectory) in natsorted(os.listdir(main_experiment_directory)):
        ...
        for (run) in natsorted(os.listdir(experiment_directory)):
            ...
            result_txt = os.path.join(experiment_directory, run) + "\\concatenated_results.txt"

            all_metrics = { # tp: true positive, tn: true negative, likewise for f = false
                "data_origin": result_txt,
                "connection_tp": [], 
                "connection_tn": [],
                "connection_fp": [],
                "connection_fn": [],
